...
opioids <- opioids[rx_start_dt %within% interval(washout_12mos_end_dt, 
...
summarize_opioids <- function(data) {
    ...
    long <- data[, .(date = seq(rx_start_dt, rx_end_dt, by = "1 day"), 
                     mme_strength_per_day, opioid, washout_12mos_end_dt), 
                 by = seq_len(nrow(data))
                 ][, seq_len := NULL][]
    
    long <- long[, days_from_mediator_start := difftime(date, washout_12mos_end_dt, units = "day")
...
saveRDS(out, file.path(drv_root, "mme_month_12mo_summary_mediator_period_only.rds"))
...
saveRDS(avg_30d_dose, file.path(drv_root, "mediator_12mo_average_daily_dose_mme_per_month.rds")) 
...
saveRDS(overall_avg_daily_dose, file.path(drv_root, "mediator_12mo_average_daily_dose_mme_overall.rds")) 
