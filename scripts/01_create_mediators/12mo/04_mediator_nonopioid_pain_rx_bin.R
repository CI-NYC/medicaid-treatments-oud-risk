...
nop[, mediator_nonopioid_pain_rx := 
      as.numeric(rowSums(nop[, .SD, .SDcols = grep("^mediator", names(nop), value = TRUE)]) > 0)]

saveRDS(nop, file.path(drv_root, "mediator_12mo_nop_binary_refactor.rds"))
