...
otl <- otl[LINE_SRVC_BGN_DT %within% interval(washout_12mos_end_dt, 
           ...
           .(BENE_ID, LINE_SRVC_BGN_DT, washout_12mos_end_dt, NDC, NDC_QTY, flag_nop)]
...
rxl <- rxl[RX_FILL_DT %within% interval(washout_12mos_end_dt, 
                                        washout_12mos_end_dt + days(182)), 
           .(BENE_ID, RX_FILL_DT, washout_12mos_end_dt, NDC, NDC_QTY, DAYS_SUPPLY, flag_nop)]

# Export ------------------------------------------------------------------

saveRDS(otl, file.path(drv_root, "mediator_12mo_otl_nonopioid_pain_rx.rds"))
saveRDS(rxl, file.path(drv_root, "mediator_12mo_rxl_nonopioid_pain_rx.rds"))
