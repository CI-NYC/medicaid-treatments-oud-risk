...
cohort <- cohort[, .(BENE_ID, washout_start_dt, washout_cal_end_dt)]
...
claims <- claims[LINE_SRVC_BGN_DT %within% interval(washout_start_dt, washout_cal_end_dt), 
                 .(BENE_ID, LINE_SRVC_BGN_DT, washout_start_dt, washout_cal_end_dt, LINE_PRCDR_CD)]

# Create indicator variable for whether or not a patient had claim in mediator period
# Right join with cohort
claims <- claims[, .(baseline_has_counseling = as.numeric(.N > 0)), by = "BENE_ID"]
...
fix <- "baseline_has_counseling"
claims[, (fix) := lapply(.SD, \(x) fifelse(is.na(x), 0, x)), .SDcols = fix]

saveRDS(claims, file.path(drv_root, "baseline_has_counseling.rds"))
