...
read_res <- function(Y, subset) {
    map_dfr(M, function(mediator) {
        file.path(drv_root, 
                  "mtp_fits", 
                  glue("fit_{gsub(' ', '_', subset)}_{Y}_outcome_fix_mediator_{mediator}.rds")) |> 
            readRDS() |> 
            tidy() |> 
...
read_diff <- function(Y, subset1, subset2) {
    map_dfr(M, function(mediator) {
        diff <- file.path(drv_root, 
                          "mtp_fits", 
                          glue("fit_{gsub(' ', '_', subset1)}_{Y}_outcome_fix_mediator_{mediator}.rds")) |> 
            readRDS() |> 
            lmtp_contrast(ref = readRDS(file.path(drv_root, "mtp_fits", glue("fit_{subset2}_{Y}_outcome_fix_no_cens.rds"))))
...
res_dcp <- file.path(drv_root, "mtp_fits", glue("fit_dcp_{Y}_outcome_fix_no_cens.rds")) |> 
    readRDS() |> 
    tidy() |> 
...
res_cp <- file.path(drv_root, "mtp_fits", glue("fit_cp_{Y}_outcome_fix_no_cens.rds")) |> 
    readRDS() |> 
    tidy() |> 
    mutate(mediator = "No censoring", .before = "estimator") |> 
...
label_counts <- function(data, subset, m) {
    subsetted <- data[disability_pain_cal == subset]
...
ragg::agg_png(
    glue("figures/mtp_{Y}_outcome_fix_disability_chronic_pain_riskdiff.png"), 
...
ragg::agg_png(
    glue("figures/mtp_{Y}_outcome_fix_disability_chronic_pain_relrisk.png"), 
    width = 7, height = 3, units = "cm", res = 600
)
...
ragg::agg_png(
    glue("figures/mtp_{Y}_outcome_fix_chronic_pain_only_riskdiff.png"), 
    width = 7, height = 3, units = "cm", res = 600
)
...
ragg::agg_png(
    glue("figures/mtp_{Y}_outcome_fix_chronic_pain_only_relrisk.png"), 
    width = 7, height = 3, units = "cm", res = 600
)
