...
dcp <- data[disability_pain_12mos_cal == "disability and chronic pain"]
setDF(dcp)

cp <- data[disability_pain_12mos_cal == "chronic pain only"]
setDF(cp)
...
saveRDS(fit_dcp, file.path(drv_root, "mtp_fits", glue::glue("fit_12mo_dcp_{Y}_no_cens.rds")))
...
saveRDS(fit_cp, file.path(drv_root, "mtp_fits", glue::glue("fit_12mo_cp_{Y}_no_cens.rds")))
